...
public class KSdist {
  ...
  public double pks(final double z) {
    if (z < 0.) throw new IllegalArgumentException("bad z in KSdist");
    if (z < 0.042) return 0.;
    if (z < 1.18) {
  ...
  public double invqks(final double q) {
    ...
    if (q <= 0. || q > 1.) throw new IllegalArgumentException("bad q in KSdist");
    if (q == 1.) return 0.;
    if (q > 0.3) {
  ...
  public double invpks(final double p) {return invqks(1.-p);}
}
