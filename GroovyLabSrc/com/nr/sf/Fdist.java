...
public class Fdist extends Beta{
  ...
  public Fdist(double nnu1, double nnu2) {
    nu1 = nnu1;
    nu2 = nnu2;
    if (nu1 <= 0. || nu2 <= 0.) throw new IllegalArgumentException("bad nu1,nu2 in Fdist");
  ...
  public double p(double f) {
    if (f <= 0.) throw new IllegalArgumentException("bad f in Fdist");
  ...
  public double invcdf(double p) {
    if (p <= 0. || p >= 1.) throw new IllegalArgumentException("bad p in Fdist");
