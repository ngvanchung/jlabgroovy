...
/* Simple stucture to represent a point in DIM dimensions */
...
public class Point extends Object{
  ...
  public Point(final int dim, final double[] xx) {
    ...
    if (dim > 1) x[1] = xx[1];
    if (dim > 2) x[2] = xx[2];
    if (dim > 3) throw new IllegalArgumentException("Point not implemented for DIM > 3");
  }
  ...
  public Point copyAssign(final Point p) {
    x = new double[p.dim()];
    for (int i=0; i<x.length; i++) x[i] = p.x[i];
    return this;
  }
  ...
  public static double dist(final Point p, final Point q) {
    if(p.dim() != q.dim()) throw new IllegalArgumentException("need same dim!");
