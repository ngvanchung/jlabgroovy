...
public class Indexx {
  ...
  // calls index and stores an index to the array arr[0..n-1]
  public Indexx(final double[] arr) {
    index(arr,arr.length);
  }
  
  public Indexx() {}   // empty constructor, see text
  ...
  public void sort(final double[] brr) {
    if (brr.length != n) throw new IllegalArgumentException("bad size in Index sort");
  ...
  // this function and the next, return the element of brr that would be in sorted postition j
  // accoring to the stored index. The vector brr is not changed
  ...
  // is in ascending order for j = 0, 1,1 .., nn-1. Also sets member value n. The input array arr is not changed. 
  public void index(final double[] arr, final int nn) {
    ...
    for (;;) {
      if (ir-l < M) {
        ...
        indx[j]=indxt;
        jstack += 2;
        if (jstack >= NSTACK) throw new IllegalArgumentException("NSTACK too small in index.");
        if (ir-i+1 >= j-l) {
          istack[jstack]=ir;
          istack[jstack-1]=i;
          ir=j-1;
