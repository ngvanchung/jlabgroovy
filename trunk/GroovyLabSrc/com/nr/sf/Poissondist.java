...
public class Poissondist extends Gamma{
  ...
  public Poissondist(double llam) {
    lam = llam;
    if (lam <= 0.) throw new IllegalArgumentException("bad lam in Poissondist");  
  }
  
  // probability density function
  public double p(int n) {
    if (n < 0) throw new IllegalArgumentException("bad n in Poissondist");
  ...
  public int invcdf(double p) {
    int n,nl,nu,inc=1;
    if (p <= 0. || p >= 1.) throw new IllegalArgumentException("bad p in Poissondist");
