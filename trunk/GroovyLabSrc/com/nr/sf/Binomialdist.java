...
public class Binomialdist extends Beta {
  ...
  // initialize with nn (sample size), and ppe (event probability)
  public Binomialdist(int nn, double ppe) {
    n = nn;
    pe = ppe;
    if (n <= 0 || pe <= 0. || pe >= 1.) throw new IllegalArgumentException("bad args in Binomialdist");
    fac = gammln(n+1.);
  }
  
  // probability density function
  public double p(int k) {
    if (k < 0) throw new IllegalArgumentException("bad k in Binomialdist");
    if (k > n) return 0.;
    return exp(k*log(pe)+(n-k)*log(1.-pe)
      +fac-gammln(k+1.)-gammln(n-k+1.));
  }
  
  // cmulative distribution function
  ...
  public int invcdf(double p) {
    int k,kl,ku,inc=1;
    if (p <= 0. || p >= 1.) throw new IllegalArgumentException("bad p in Binomialdist");
