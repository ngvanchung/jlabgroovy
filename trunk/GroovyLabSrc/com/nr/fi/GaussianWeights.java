...
public class GaussianWeights {
   ...
   * ten times at interior points in the range of integration.
      */
  public static double qgaus(final UniVarRealValueFun func, final double a, final double b){
    double xm=0.5*(b+a);
    double xr=0.5*(b-a);
    double s=0;
    for (int j=0;j<5;j++) {
   ...
   * @param alf
   */
  public static void gaulag(final double[] x, final double[] w, final double alf) {
    final int MAXIT=10;
    final double EPS=1.0e-14;
    int i,its,j;
    ...
    for (i=0;i<n;i++) {
      ...
      if (its >= MAXIT) throw new IllegalArgumentException("too many iterations in gaulag");
      x[i]=z;
  ...
  public static void gauher(final double[] x, final double[] w) {
    ...
    for (i=0;i<m;i++) {
      ...
      // if (its >= MAXIT) throw new IllegalArgumentException("too many iterations in gauher");
      x[i]=z;
      x[n-1-i] = -z;
      w[i]=2.0/(pp*pp);
      w[n-1-i]=w[i];
   ...
   * @param bet
   */
  public static void gaujac(final double[] x, final double[] w, final double alf, final double bet) {
    final int MAXIT=10;
    final double EPS=1.0e-14;
    int i,its,j;
    ...
    for (i=0;i<n;i++) {
      ...
      if (its > MAXIT) throw new IllegalArgumentException("too many iterations in gaujac");
      x[i]=z;
