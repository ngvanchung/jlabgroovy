...
public class Fourier {
   ...
   * the correction factor that multiplies  the DFT and the endpoint correction to be added. Input is the
   * angular frequency w, stepsize delta, lower and upper limits of  the integral a and b, 
   * while the array endpts contains the first 4 and last 4 function values. The correction factor 
   * W(θ)  is returned  as corfac, while the real and imaginary parts of the endpoint correction
   * are returned as corre and corim.
   */
  public static void dftcor(final double w, final double delta, final double a, final double b,
      final double[] endpts, final doubleW corre, final doubleW corim, final doubleW corfac) {
    double a0i,a0r,a1i,a1r,a2i,a2r,a3i,a3r,arg,c,cl,cr,s,sl,sr,t,t2,t4,t6,
   ...
   * lags are in ans[0] (zero lag) on up to ans[n/2-1]. Sign convention of this
   * routine: if data1 lags data2, i.e., is shifted to the right of it, then ans
   * will show a peak at positive lags.
   * 
   * @param data1
   * @param data2
   * @param ans
  ...
  /*
   var N = 2 ^^^ 10
   var t = linspace(-2, 2, N)
   var data = 0.02*cos(0.55*t)
   data = data+3*vrandn(N)
   var ans = new Array[Double](t.length)
   com.nr.sp.Fourier.correl(data, data, ans)
   
   plot(data, "Original data"); hold("on"); plot(ans, Color.GREEN, "Result of correlation")
*/
  public static void correl(final double[] data1, final double[] data2, final double[] ans) {
    ...
    FFT.realft(ans,1);   // Transform both data vectors
    FFT.realft(temp,1);  
    ...
    FFT.realft(ans,-1);    // Inverse transform gives correlation
  }
   ...
   * user-supplied zero padding) with a response function respns[0..m-1], where
   * m is an odd integer <= n. The response function must be stored in wraparound
   * order: The first half of the array respns contains the impulse response
   ...
   * element respns[m-1]. On input isign is C1 for convolution, 1 for
   * deconvolution. The answer is returned in ans[0..n-1]. n must be an integer
   * power of 2.
   * 
     */
  ...
  /*
  var N = 2 ^^^ 10
   var t = linspace(-5, 5, N)
   var data = 2.3*cos(0.55*t)
   var respns = Array(1.0, 0.9, 0.8, 0.8, 0.9, 1.0)
   var isign = 1  // convolve
   var ans = new Array[Double](t.length)
   com.nr.sp.Fourier.convlv(data, respns, isign, ans)
   
   plot(data, "Original data"); hold("on"); plot(ans, Color.GREEN, "Result of convolution")
   */
  public static void convlv(final double[] data, final double[] respns, final int isign,
      final double[] ans) {
    ...
    for (i=0;i<n;i++)
      ans[i]=data[i];
    FFT.realft(ans,1);   // FFT both arrays
    FFT.realft(temp,1);
    no2=n>>1;
    if (isign == 1) {
      for (i=2;i<n;i+=2) {   // Multiply FFTs top convolve
        tmp=ans[i];
        ans[i]=(ans[i]*temp[i]-ans[i+1]*temp[i+1])/no2;
        ans[i+1]=(ans[i+1]*temp[i]+tmp*temp[i+1])/no2;
      }
      ...
      for (i=2;i<n;i+=2) {   // Divide FFTs to deconvolve
        if ((mag2=SQR(temp[i])+SQR(temp[i+1])) == 0.0)
          throw new IllegalArgumentException("Deconvolving at response zero in convlv");
        tmp=ans[i];
      ...
      ans[0]=ans[0]/temp[0]/no2;
      ans[1]=ans[1]/temp[1]/no2;
    } else throw new IllegalArgumentException("No meaning for isign in convlv");
    FFT.realft(ans,-1);   // Inverse transform back to time domain
  }

  /**
   Given an array yy[0..n-1], extrapolate (spread) a value y into m actual array elements
   that best approximate the "fictional" (i.e., possibly noninteger) array element nmber x. The weights used are coefficients 
   of the Lagrange interpolating polynomial.
   */
  public static void spread(final double y, final double[] yy, final double x, final int m) {
    int[] nfac={0,1,1,2,6,24,120,720,5040,40320,362880};
    int ihi,ilo,ix,j,nden,n=yy.length;
    double fac;
    if (m > 10) throw new IllegalArgumentException("factorial table too small in spread");
    ix=(int)(x);
    if (x == (ix)) yy[ix-1] += y;
    else {
   ...
   * larger), this routine fills array px[0..nout-1] with an increasing sequence
   * of frequencies (not angular frequencies) up to hifac times the "average"
   * Nyquist frequency, and fills array py[0..nout-1] with the values of the
   * Lomb normalized periodogram at those frequencies. The arrays x and y are
   * not altered. The vectors px and py are resized to nout (eq. 13.8.9) if
   * their initial size is less than this; otherwise, only their first nout
   * components are filled. The routine also returns jmax such that py[jmax] is
   * the maximum element in py, and prob, an estimate of the significance of
   * that maximum against the hypothesis of random noise. A small value of prob
  ...
  public void fasper(final double[] x, final double[] y, final double ofac, final double hifac) {
    ...
    Moment.avevar(y,ave,var);   // Compute the mean, variance, and range of the data.
    if (var.val == 0.0) throw new IllegalArgumentException("zero variance in fasper");
    xmin=x[0];
    xmax=xmin;
    for (j=1;j<n;j++) {
    ...
    xdif=xmax-xmin;
    double[] wk1 = new double[nwk];    // Zero the workspaces.
    double[] wk2 = new double[nwk];
    fac=nwk/(xdif*ofac);
    fndim=nwk;
    for (j=0;j<n;j++) {    // Extrapolate the data into the workspaces.
    ...
    FFT.realft(wk1,1);     // Take the Fast Fourier Transforms
    FFT.realft(wk2,1);
    df=1.0/(xdif*ofac);
    pmax = -1.0;
    for (k=2,j=0;j<nout;j++,k+=2) {    // Compute the Lomb value for each frequency.
   ...
   * ofac (a typical value being 4 or larger), this routine fills array  px[0..nout-1] with an 
   * increasing sequence of frequencies (not angular  frequencies) up to hifac times the "average" Nyquist
   * frequency, and fills  array py[0..nout-1] with the values of the Lomb normalized periodogram at
  ...
  public void period(final double[] x, final double[] y, final double ofac, final double hifac) {
    ...
    Moment.avevar(y,ave,var);  // Get mean and variance of the input data
    if (var.val == 0.0) throw new IllegalArgumentException("zero variance in period");
    xmax=xmin=x[0];     // Go through data to get the range of abscissas 
    for (j=0;j<n;j++) {
      if (x[j] > xmax) xmax=x[j];
      if (x[j] < xmin) xmin=x[j];
    }
    ...
    for (j=0;j<n;j++) {    // Initialize values for the trigonometric recurrences at each data point
    ...
    for (i=0;i<nout;i++) {   // Main loop over the frequencies to be evaluated
      px[i]=pnow;
      sumsh=sumc=0.0;
      for (j=0;j<n;j++) {   // First, loop over the data to get r and related quantities
        c=wr[j];
        s=wi[j];
        sumsh += s*c;
        sumc += (c-s)*(c+s);
      ...
      for (j=0;j<n;j++) {
        ...
        sumsy += yy*ss;
        sumcy += yy*cc;
        wr[j]=((wtemp=wr[j])*wpr[j]-wi[j]*wpi[j])+wr[j];   // Update the trigonometric recurrences
        wi[j]=(wi[j]*wpr[j]+wtemp*wpi[j])+wi[j];
      }
      py[i]=0.5*(sumcy*sumcy/sumc+sumsy*sumsy/sums)/var.val;
      if (py[i] >= pymax){
        jmax=i;
        pymax=py[jmax];
      }
   ...
   * @param d
   */
  public static void memcof(final double[] data, final doubleW xms, final double[] d) {
    ...
    for (k=0;k<m;k++) {
      ...
      /*
       The algorithm is recursive, building up the answer for larger and larger values of m until the desired value
       is reached. At this point in the algorithm, one could return the vector d and scalar xms for a set of LP coefficients with k 
       (rather than m) terms
       */
      if (k == m-1)
        return;
  ...
  public static void fixrts(final double[] d) {
    ...
    for (j=0;j<m;j++)     // The polynomial coefficients are guaranteed to be real, so we need to return the real part
      d[m-1-j] = -a[j].re();  // as new LP coefficients
  }
   ...
   * @param future
   */
  public static void predic(final double[] data, final double[] d, final double[] future) {
    ...
    for (j=0;j<nfut;j++) {
      discrp=0.0;
      // This is where you would put in known discrepancy if you were reconstructing a function by
      // linear predictive coding rather than extrapolating a function by linear prediction. See text
      sum=discrp;
   ...
   * @return
   */
  public static double evlmem(final double fdt, final double[] d, final double xms) {
    int i;
